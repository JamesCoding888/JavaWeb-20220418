...
public class MessageServiceLineSticker {	
		// token 不設定在 Service 而是讓使用者在網頁輸入
		// private String token = "WOMxnP8lAUhb3vSCNBh57HFAkJepHMlxdvTjsNc2"; 
		private String token = "";		
		
		public MessageServiceLineSticker(String token) {
			this.token = token;			
		}
		...
		public int pushMessageAndSticker(String content, String stickerPackageId, String stickerId) {
			...
			try {
				// 將要發送的訊息轉為 byte[] 並寫編碼
				byte[] postData = ("message=" + content + 
						           "&stickerPackageId=" + stickerPackageId + 
								   "&stickerId=" + stickerId).getBytes("UTF-8");
				// 建立 URL 物件
				URL url = new URL(lineNotifyUrl);
				// 建立 HttpURLConnection 連線物件
				HttpURLConnection conn = (HttpURLConnection)url.openConnection();
		...
		public int pushMessageAndWebImage(String content, String webImageUrl) {
			...
			try {
				// 將要發送的訊息轉為 byte[] 並寫編碼
				byte[] postData = ("message=" + content + "&imageThumbnail=" + webImageUrl + "&imageFullsize="
						+ webImageUrl).getBytes("UTF-8");
				// 建立 URL 物件
				URL url = new URL(lineNotifyUrl);
				// 建立 HttpURLConnection 連線物件
				HttpURLConnection conn = (HttpURLConnection) url.openConnection();
